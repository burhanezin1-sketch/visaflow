-- Kural 35: Almanya + Transit Vize — çalışan SGK belgeleri + Almanca tercüme ibaresi
...
BEGIN
  ...
  -- 35. Almanya + Transit Vize: çalışan SGK belgeleri + Almanca tercüme ibaresi
  IF p_country = 'Almanya' AND p_visa_type = 'Transit Vize' THEN
    IF p_occupation = 'calisan' THEN
      IF NOT EXISTS (
        SELECT 1 FROM public.user_submitted_docs
        WHERE  application_id = p_application_id AND doc_name ILIKE '%SGK İşe Giriş%'
      ) THEN
        INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
        VALUES
          (p_application_id, 'SGK İşe Giriş Bildirgesi (e-devlet, barkodlu)', 'digital', 'pending'),
          (p_application_id, 'SGK Hizmet Dökümü (e-devlet, barkodlu, tüm sigorta geçmişini kapsayan)', 'digital', 'pending');
      END IF;
    END IF;
