-- Amerika post-processing güncellendi:
-- sigorta sil + uçak rezervasyonu sil + banka adı düzelt eklendi
...
BEGIN
  ...
  -- 5. Italya: banka dokumu adini duzelt
  ...
  -- 7. İngiltere: banka adını düzelt ve sigorta sil
  ...
  -- 8. Japonya: sigorta sil ve banka adını düzelt
  ...
  -- 9. Güney Kore: sigorta sil ve banka adını düzelt
  ...
  -- 10. Amerika Birlesik Devletleri ozel kurallari
  IF p_country = 'Amerika Birleşik Devletleri' THEN

    -- Schengen, 30.000 EUR ve Ulusal (D Tipi) evraklarini sil
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
    ...
    -- Biyometrik fotografi Amerika standardina cevir (50x50 mm)
    UPDATE public.user_submitted_docs
