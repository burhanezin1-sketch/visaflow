-- Ek postprocess: calisma vizesi visitor sil, ev hanimi banka sil, tercume fix
...
BEGIN
  ...
  IF p_occupation = 'emekli' THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%Şahsi Vize Talep Dilekçesi%'
    ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%Emeklilik Belgesi%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
  ...
  IF p_occupation = 'ogrenci' THEN
    ...
    DELETE FROM public.user_submitted_docs
    WHERE (
      application_id = p_application_id AND LOWER(doc_name) LIKE '%şahsi%banka%'
    ) OR (
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%Sponsor Dilekçesi%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
  ...
  IF p_occupation = 'devlet_memuru' THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%Görev Belgesi%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES
  ...
  IF p_occupation IN ('ev_hanimi_meslek', 'ev_hanimi') THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%Sponsorluk Yazısı%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
  ...
  IF p_country = 'Almanya' THEN
    ...
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%banka hesap dökümü%' AND doc_name ILIKE '%İmza Sirküleri%';
  END IF;
  ...
  IF p_country = 'Fransa' THEN
    IF EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%İngilizce veya Fransızca%'
    ) THEN
      DELETE FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
  ...
  IF p_country = 'Amerika Birleşik Devletleri' THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%DS-160%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
  ...
  IF p_country IN ('Kanada', 'İngiltere', 'İrlanda') THEN
    IF p_country = 'Kanada' THEN
      ...
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%fotoğraf%' AND doc_name ILIKE '%35x45%';
    END IF;
  ...
  IF p_visa_type = 'Çalışma/İş Vizesi' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%visitor visa formu%';
  END IF;
  ...
  -- 27. İngiltere/Kanada/İrlanda: ikametgah dahil tercüme ibaresi (ek geçiş)
