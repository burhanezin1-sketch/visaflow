-- Rule Engine entegrasyonu: eksik meslek/vize-turu kurallari eklendi
...
BEGIN
  ...
  -- 4. Emekli: ozel sektor evraklarini sil, sahsi dilekce + emeklilik belgesi ekle
  ...
  -- 5. Ogrenci: isveren/aile evraklarini sil, sponsor adini duzelt, sponsor dilekce ekle
  ...
  -- 6. Devlet Memuru: ozel sektor evraklarini sil, maas bordronu guncelle, gorev belgesi ekle
  ...
  -- 7. Ev Hanimi + Calismiyor: sirket/maas/sgk sil, aile cuzdani + sponsorluk ekle
  IF p_occupation IN ('ev_hanimi_meslek', 'ev_hanimi') THEN
    ...
    IF p_occupation = 'ev_hanimi_meslek' AND NOT EXISTS (
      ...
      VALUES (
        p_application_id,
        'Aile cüzdanı / nikah cüzdanı fotokopisi (sponsorla olan evlilik ve akrabalık bağını kanıtlar)',
        'digital',
        'pending'
      );
    END IF;
  ...
  -- 8. Schengen disi ulkelerde calisan: isveren izin yazisi adini duzelt
  ...
  -- 9. Almanya: banka dokumu adini duzelt
  ...
  -- 10. Italya: banka dokumu adini duzelt
  ...
  -- 11. Fransa: ozel dilekce varsa genel dilekceyi sil
  ...
  -- 12. İngiltere: banka adını düzelt ve sigorta sil
  ...
  -- 13. Japonya: sigorta sil ve banka adını düzelt
  ...
  -- 14. Güney Kore: sigorta sil ve banka adını düzelt
  ...
  -- 15. Kanada: sigorta sil + ucak rezervasyonu sil + banka adı düzelt
  ...
  -- 16. Amerika Birlesik Devletleri: Schengen/sigorta/ucak sil, foto+banka rename, DS-160 ekle
  ...
  -- ---------------------------------------------------------
  -- 17-22. VİZE TÜRÜ BAZLI EK EVRAKLAR
  -- ---------------------------------------------------------

  -- 17. Ticari/İş Gezisi: görevlendirme yazısı + ticari davetiye
  IF p_visa_type = 'Ticari/İş Gezisi' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Görevlendirme Yazısı%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (
        p_application_id,
        'Şirket/Kurum Görevlendirme Yazısı (görevi ve seyahat amacını belirten ıslak imzalı/kaşeli yazı)',
        'digital',
        'pending'
      );
    END IF;
    ...
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Ticari Davetiye%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (
        p_application_id,
        'Resmi Ticari Davetiye (karşı firmadan veya fuar organizasyonundan gelen davet mektubu)',
        'digital',
        'pending'
      );
    END IF;
  ...
  -- 18. Aile/Arkadaş Ziyareti: belediye onaylı davetiye
  IF p_visa_type = 'Aile/Arkadaş Ziyareti' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Belediye%Davetiye%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (
        p_application_id,
        'Belediye / Yabancılar Dairesi Onaylı Orijinal Davetiye (davet eden kişinin bulunduğu ülkeden resmi onaylı belge)',
        'digital',
        'pending'
      );
    END IF;
  ...
  -- 19. Aile Birleşimi Vizesi: uluslararası evlenme kayıt
  IF p_visa_type = 'Aile Birleşimi Vizesi' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Uluslararası Evlenme%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (
        p_application_id,
        'Uluslararası Evlenme Kayıt Örneği (Formül B) ve Dil Yeterlilik Belgesi (gerekli ülkeler için A1 sertifikası)',
        'digital',
        'pending'
      );
    END IF;
  ...
  -- 20. Eğitim/Öğrenci: okul kabul mektubu
  IF p_visa_type = 'Eğitim/Öğrenci' THEN
    IF NOT EXISTS (
      ...
      VALUES (
        p_application_id,
        'Kabul Mektubu / Akreditasyon Belgesi (gidilecek okul veya üniversiteden resmi kayıt onayı)',
        'digital',
        'pending'
      );
    END IF;
  ...
  -- 21. Tedavi/Sağlık Vizesi: sağlık raporu + hastane kabul belgesi
  IF p_visa_type = 'Tedavi/Sağlık Vizesi' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Sağlık Raporu%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (
        p_application_id,
        'Sağlık Raporu ve Karşı Hastaneden Kabul Belgesi (tedavi detaylarını ve maliyet üstlenimini gösteren evraklar)',
        'digital',
        'pending'
      );
    END IF;
  ...
  -- 22. Kültürel Vize: etkinlik / davet katılım belgesi
  IF p_visa_type = 'Kültürel Vize' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Etkinlik%Davet%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (
        p_application_id,
        'Etkinlik / Davet Katılım Belgesi (kültürel veya sportif organizasyon davetiyesi ve program)',
        'digital',
        'pending'
      );
    END IF;
