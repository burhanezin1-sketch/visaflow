-- get_visa_documents() - SQL Editor uyumlu versiyon
-- Degisiklik: box-drawing karakterleri (U+2550) yorumlardan kaldirildi.
-- String literal'lardaki Turkce karakterler ve Euro isareti aynen korundu.
...
BEGIN
  ...
  -- Filtre A: Schengen disi ulkelerde 30.000 EUR sigortayi atla
  -- Filtre B: Transit Vize'de ucak/bilet evraklarini atla
  -- ---------------------------------------------------------
  ...
  -- basvuru formu / davet dedup mantigi korundu
  -- ---------------------------------------------------------
  INSERT INTO public.user_submitted_docs
    (application_id, doc_name, delivery_type, status)
  SELECT p_application_id, doc_name, delivery_type, 'pending'
  ...
  -- 1. Mukerrer sigorta: 'tum Schengen' versiyonu varsa
  --    generic 'Seyahat saglik sigortasi' satirini sil
  IF EXISTS (
  ...
  -- 5. Amerika Birlesik Devletleri ozel kurallari
  IF p_country = 'Amerika Birleşik Devletleri' THEN

    -- 5a. Schengen, 30.000 EUR ve Ulusal (D Tipi) evraklarini sil
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
    ...
    -- 5b. Biyometrik fotografi Amerika standardina cevir (50x50 mm)
    UPDATE public.user_submitted_docs
