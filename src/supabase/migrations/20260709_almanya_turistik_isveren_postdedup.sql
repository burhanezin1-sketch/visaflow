-- Kural 46 güncelleme: Almanya + Turistik + işveren bloğu DEDUP sonrasına taşındı, DELETE güçlendirildi
...
BEGIN
  ...
  -- 46. Almanya + Turistik + isveren: antetli dilekce kesin sil (DEDUP sonrasi), sahsi dilekce + Bagkur ekle
  IF p_country = 'Almanya' AND p_visa_type = 'Turistik' AND p_occupation = 'isveren' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%antetli%'
      AND  doc_name ILIKE '%dilekçe%';
