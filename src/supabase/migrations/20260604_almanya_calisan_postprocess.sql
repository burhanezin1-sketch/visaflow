-- Almanya + calisan: dilekce sil + isveren izin yazisini sadelessstir
...
BEGIN
  ...
  -- 4. Schengen disi ulkelerde calisan: isveren izin yazisi adini duzelt
  IF p_country IN ('Amerika Birleşik Devletleri', 'Kanada', 'İngiltere', 'Japonya', 'Güney Kore')
     AND p_occupation = 'calisan' THEN
    ...
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%İşveren İzin%';
  END IF;

  -- 5. Almanya + calisan: dilekce sil + isveren yazisini sadelessstir
  IF p_country = 'Almanya' AND p_occupation = 'calisan' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%vize talep dilekçesi%';

    UPDATE public.user_submitted_docs
    SET    doc_name = 'Şirket İzin ve Görev Yazısı (şirketin antetli kağıdına, yetkili imzalı ve kaşeli; görev, izin tarihleri ve maaş bilgisi yer almalıdır)'
    WHERE  application_id = p_application_id
      AND  (doc_name ILIKE '%İşveren İzin%' OR doc_name ILIKE '%Görev Yazısı%');
  END IF;

  -- 6. Almanya: banka dokumu adini duzelt
  ...
  -- 7. Italya: banka dokumu adini duzelt
  ...
  -- 8. Fransa: ozel dilekce varsa genel dilekceyi sil
  ...
  -- 9. İngiltere: banka adını düzelt ve sigorta sil
  ...
  -- 10. Japonya: sigorta sil ve banka adını düzelt
  ...
  -- 11. Güney Kore: sigorta sil ve banka adını düzelt
  ...
  -- 12. Kanada: sigorta sil + ucak rezervasyonu sil + banka adı düzelt
  ...
  -- 13. Amerika Birlesik Devletleri ozel kurallari
