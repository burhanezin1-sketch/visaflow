-- Kural 47: Almanya + Turistik + çalışmıyor — sponsorluk temizlik + dilekçe + banka ekle (DEDUP sonrası)
...
BEGIN
  ...
  -- 47. Almanya + Turistik + calismiyor: sponsorluk temizligi + dilekce + banka ekle (DEDUP sonrasi)
  IF p_country = 'Almanya' AND p_visa_type = 'Turistik' AND p_occupation = 'calismiyor' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
        doc_name ILIKE '%masraf taahhütname%'
    ...
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%Sponsorun İşveren%';

    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
        doc_name ILIKE '%aile cüzdanı%'
        OR doc_name ILIKE '%nikah cüzdanı%'
      );

    INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
    SELECT p_application_id, 'Sponsorluk dilekçesi (sponsor tarafından konsolosluğa hitaben yazılmış, yakınlık derecesini ve tüm masrafları karşılayacağını belirten ıslak imzalı mektup)', 'digital', 'pending'
    WHERE NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id AND doc_name ILIKE '%Sponsorluk dilekçesi%'
    );
    ...
    INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
    SELECT p_application_id, 'Başvuru sahibine ait son 3 aylık şahsi banka hesap dökümü (banka kaşeli ve ıslak imzalı, varsa aktif kullanılan hesap)', 'digital', 'pending'
    WHERE NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
