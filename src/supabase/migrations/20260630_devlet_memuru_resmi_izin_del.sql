-- Kural 6 ek: devlet_memuru bloğuna standalone Resmi İzin Belgesi temizliği eklendi
-- Görev belgesi içeren (birleşik) kayıtlar korunur; tekil Resmi İzin Belgesi silinir.
...
BEGIN
  ...
  IF p_occupation = 'devlet_memuru' THEN
    ...
    -- Tekil Resmi İzin Belgesi maddesini sil — birleşik 'Görev belgesi ve resmi izin belgesi' korunur
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
  ...
  -- 42. Almanya + Turistik: konaklama rezervasyonu + devlet memuru görev belgesi standartlaştırma
