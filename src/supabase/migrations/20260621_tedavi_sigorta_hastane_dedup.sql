-- Kural 34: Tedavi/Sağlık Vizesi — mükerrer sigorta + hastane davetiye dedup
-- Not: ctid tid tipindedir, MIN() aggregate kullanılamaz.
--      COUNT>1 kontrolü ve MIN(ctid) kaldırıldı; DELETE...USING a.ctid > b.ctid kullanıldı.
...
BEGIN
  ...
  IF p_visa_type ILIKE '%Tedavi%' OR p_visa_type ILIKE '%Sağlık%' THEN
    -- Detaylı Schengen sigorta varsa sade olanı sil
    IF EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
    ...
    -- Mükerrer hastane kabul/randevu belgelerini teke düşür (en eski kayıt kalır)
    DELETE FROM public.user_submitted_docs a
    USING  public.user_submitted_docs b
    WHERE  a.application_id = p_application_id
      AND  b.application_id = p_application_id
    ...
    -- Kalan hastane belgesini standart ada güncelle
    UPDATE public.user_submitted_docs
