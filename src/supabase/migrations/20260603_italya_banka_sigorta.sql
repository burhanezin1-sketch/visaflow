-- İtalya: banka adı düzeltmesi + sigorta sıralama iyileştirmesi
-- Sigorta için KATMAN 1 ORDER BY değiştirildi (post-processing değil):
-- UUID'ler zaman damgalı olduğu için INSERT sırası = görüntülenme sırası.
-- İtalya'da sigortayı order_num 4.5 konumuna alarak fotoğrafın hemen arkasına taşır.
...
BEGIN
  ...
  -- Italya: sigorta order_num 8 -> 4.5 (fotografin hemen arkasi)
  -- ---------------------------------------------------------
  ...
  -- 5. Italya: banka dokumu adini duzelt (Imza Sirkuleri ibaresi kaldir)
  ...
  -- 6. Fransa: ozel dilekce varsa genel dilekceyi sil
  ...
  -- 7. Amerika Birlesik Devletleri ozel kurallari
  IF p_country = 'Amerika Birleşik Devletleri' THEN

    -- 7a. Schengen, 30.000 EUR ve Ulusal (D Tipi) evraklarini sil
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
    ...
    -- 7b. Biyometrik fotografi Amerika standardina cevir (50x50 mm)
    UPDATE public.user_submitted_docs
