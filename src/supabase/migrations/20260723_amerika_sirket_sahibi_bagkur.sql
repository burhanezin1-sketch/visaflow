-- Kural 58: Amerika + sirket_sahibi: 4B/Bağkur tescil ve SGK hizmet dökümü ekle (DEDUP sonrası)
...
BEGIN
  ...
  IF p_country = 'Kanada' THEN
    ...
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%kametgah%';
  END IF;

  -- 54. Turistik global fallback: YALNIZCA Schengen ulkelerinde ucak + konaklama garantile
  -- Kanada, Amerika, İngiltere, Japonya, Güney Kore bu listede YOK
  IF p_visa_type = 'Turistik' AND p_country = ANY(ARRAY[
    ...
    INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
    SELECT p_application_id, 'Gidiş-dönüş uçak bileti rezervasyonu', 'digital', 'pending'
    WHERE NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
    ...
    INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
    SELECT p_application_id, 'Otel / konaklama rezervasyonu (seyahat tarihleri ile uyumlu)', 'digital', 'pending'
    WHERE NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%konaklama rezervasyonu%'
  ...
  -- 55. Kanada + Egitim/Ogrenci + ogrenci: finansal evrak enjeksiyonu + akademik belge Ingilizce notu (DEDUP sonrasi)
  IF p_country = 'Kanada' AND p_visa_type = 'Eğitim/Öğrenci' AND p_occupation = 'ogrenci' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Kanada için yeterli bakiye%'
    ) THEN
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES
        (p_application_id, 'Sponsor veya şahsın son 3 aylık şahsi banka hesap dökümü (banka kaşeli ve ıslak imzalı, Kanada için yeterli bakiye gösteren)', 'digital', 'pending'),
        (p_application_id, 'Sponsorun gelir / işyeri belgeleri (maaş bordrosu, vergi levhası veya emeklilik belgesi)', 'digital', 'pending');
    END IF;

    UPDATE public.user_submitted_docs
    SET    doc_name = doc_name || ' (İngilizce olarak okuldan alınmalı veya e-devletten İngilizce barkodlu indirilmelidir)'
    WHERE  application_id = p_application_id
      AND (
        doc_name ILIKE '%Öğrenci belgesi%'
        OR doc_name ILIKE '%Transkript%'
      )
      AND  doc_name NOT ILIKE '%İngilizce olarak%';
  END IF;

  -- 56. Kanada + Turistik: konaklama ve ucak rezervasyonunu sil (DEDUP sonrasi — IRCC portali yeterli)
  IF p_country = 'Kanada' AND p_visa_type = 'Turistik' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
        doc_name ILIKE '%konaklama rezervasyonu%'
        OR doc_name ILIKE '%uçak bileti rezervasyonu%'
      );
  END IF;

  -- 57. Amerika + Turistik + ogrenci: sponsorluk taahhütnamesi ekle (DEDUP sonrasi)
  IF p_country = 'Amerika Birleşik Devletleri' AND p_occupation = 'ogrenci' THEN
    INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
    SELECT p_application_id,
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Sponsorluk taahhütnamesi%'
    );
  END IF;

  -- 58. Amerika + sirket_sahibi: 4B/Bagkur tescil ve SGK hizmet dokumu ekle (DEDUP sonrasi)
  IF p_country = 'Amerika Birleşik Devletleri' AND p_occupation = 'sirket_sahibi' THEN
    INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
    SELECT p_application_id,
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
        AND  doc_name ILIKE '%Bağkur tescil%'
    );
  END IF;
