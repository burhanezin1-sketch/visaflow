...
BEGIN
  ...
  -- 3. Amerika Birleşik Devletleri özel kuralları
  IF p_country = 'Amerika Birleşik Devletleri' THEN

    -- 3a. Schengen, 30.000 € ve Ulusal (D Tipi) evrakları sil
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
    ...
    -- 3b. Birleşik şirket paketi varsa tekil şirket evraklarını sil
    IF EXISTS (
      SELECT 1 FROM public.user_submitted_docs
      WHERE  application_id = p_application_id
    ...
    -- 3c. Biyometrik fotoğrafı Amerika standardına çevir (50x50 mm)
