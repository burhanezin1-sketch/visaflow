-- Yeni meslekler: ev_hanimi_meslek + devlet_memuru
-- visa_package_rules + get_visa_documents postprocess

-- ---------------------------------------------------------
-- 1. visa_package_rules: ev_hanimi_meslek → ev_hanimi paketi
-- ---------------------------------------------------------
INSERT INTO public.visa_package_rules (visa_type, occupation, package_name, priority, is_active)
SELECT visa_type, 'ev_hanimi_meslek', 'ev_hanimi', 1, true
FROM   public.visa_package_rules
WHERE  occupation = 'calismiyor'
  AND  is_active  = true
ON CONFLICT DO NOTHING;

-- ---------------------------------------------------------
-- 2. visa_package_rules: devlet_memuru → calisan paketi
-- ---------------------------------------------------------
INSERT INTO public.visa_package_rules (visa_type, occupation, package_name, priority, is_active)
SELECT visa_type, 'devlet_memuru', 'calisan', 1, true
FROM   public.visa_package_rules
WHERE  occupation = 'calisan'
  AND  is_active  = true
ON CONFLICT DO NOTHING;

-- ---------------------------------------------------------
-- 3. get_visa_documents: postprocess eklenmiş yeni versiyon
-- ---------------------------------------------------------
...
BEGIN
  ...
  -- İtalya: sigorta order_num 8 -> 4.5 (fotografin hemen arkasi)
  -- ---------------------------------------------------------
  ...
  -- 4. Emekli: sirket dilekçesini sil, sahsi dilekce ekle
  IF p_occupation = 'emekli' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%Şirket antetli kağıdına%';
  ...
  -- 5. Ogrenci: isveren izin yazisini sil, sahsi banka sil, sponsor adini duzelt
  ...
  -- 6. Devlet Memuru: sirket evraklarini sil, gorev belgesi ekle
  ...
  -- 7. Ev Hanimi Meslek: sirket/maas/sgk evraklarini sil, aile cuzdani ekle
  IF p_occupation = 'ev_hanimi_meslek' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND (
        doc_name ILIKE '%Şirket antetli%'
  ...
  -- 16. Amerika Birlesik Devletleri ozel kurallari
