-- İngiltere foto standardı + nufus ILIKE pattern + tercüme blok güncellendi
...
BEGIN
  ...
  -- 24. Kanada/İngiltere/İrlanda: foto standardı + tercüme ibaresi
  IF p_country IN ('Kanada', 'İngiltere', 'İrlanda') THEN
    IF p_country = 'Kanada' THEN
      ...
      WHERE  application_id = p_application_id AND doc_name ILIKE '%fotoğraf%' AND doc_name ILIKE '%35x45%';
    END IF;
  ...
  -- 27. İngiltere/Kanada/İrlanda: ek tercüme geçişi (sicil dahil)
  IF p_country IN ('İngiltere', 'Kanada', 'İrlanda') THEN
    UPDATE public.user_submitted_docs
    SET    doc_name = doc_name || ' (İngilizce mütercim tercüman onaylı çevirisi ile birlikte)'
    WHERE  application_id = p_application_id
      AND (
        doc_name ILIKE '%ikametgah%' OR doc_name ILIKE '%ikamet%'
        OR doc_name ILIKE '%nüfus%'  OR doc_name ILIKE '%nufus%'
        OR doc_name ILIKE '%kimlik%' OR doc_name ILIKE '%sicil%'
      )
      AND  doc_name NOT ILIKE '%tercüme%'
      AND  doc_name NOT ILIKE '%tercüman%';
  END IF;
