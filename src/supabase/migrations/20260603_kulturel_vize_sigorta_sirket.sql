-- ============================================================
-- 20260603_kulturel_vize_sigorta_sirket.sql
--
-- 1. occupation_doc_packages: calisan paketine şirket evrakları
-- 2. get_visa_documents(): Kültürel Vize + calisan için
--    post-insert duplicate sigorta temizliği
-- ============================================================


-- ── 1. occupation_doc_packages ────────────────────────────────
INSERT INTO public.occupation_doc_packages
  (occupation, doc_name, delivery_type, order_num)
VALUES
  ('calisan',
   'Şirket evrakları (güncel vergi levhası, imza sirküleri, ticaret sicil gazetesi, faaliyet belgesi)',
   'digital', 106)
...
BEGIN
  ...
  -- Kültürel Vize + calisan: duplicate seyahat sağlık sigortası temizle
  IF p_visa_type = 'Kültürel Vize' AND p_occupation = 'calisan' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
  ...
  -- Tüm vize türleri: duplicate seyahat sağlık sigortası temizle
  DELETE FROM public.user_submitted_docs
  WHERE  application_id = p_application_id
    AND  doc_name ILIKE '%seyahat sağlık sigortası%'
    AND  id NOT IN (
