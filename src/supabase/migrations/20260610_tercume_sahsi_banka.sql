-- Ev hanimi sahsi banka temizligi + Kanada/Ingiltere/Irlanda tercume ibaresi
...
BEGIN
  ...
  -- 24. Kanada/İngiltere/İrlanda: resmi belgelere tercüme ibaresi ekle
  IF p_country IN ('Kanada', 'İngiltere', 'İrlanda') THEN
    ...
    WHERE  application_id = p_application_id
      AND (
        LOWER(doc_name) LIKE '%nüfus%'
        OR LOWER(doc_name) LIKE '%ikametgah%'
        OR LOWER(doc_name) LIKE '%kimlik%'
        OR LOWER(doc_name) LIKE '%sicil%'
      )
      AND  LOWER(doc_name) NOT LIKE '%tercüme%'
      AND  LOWER(doc_name) NOT LIKE '%tercüman%';
  END IF;
