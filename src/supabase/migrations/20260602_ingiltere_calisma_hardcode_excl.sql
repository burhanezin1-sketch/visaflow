-- ============================================================
-- 20260602_ingiltere_calisma_hardcode_excl.sql
--
-- 1. user_submitted_docs: İngiltere + Çalışma/İş Vizesi
--    başvurularındaki yanlış kayıtları temizle
-- 2. get_visa_documents(): tüm 3 katmana hard-coded exclusion
--    listesi ekle — exact doc_name eşleşmesi, ANY() ile
-- ============================================================


-- ── 1. Mevcut yanlış kayıtları temizle ───────────────────────
...
BEGIN
  ...
  WHERE
    ...
    -- Filtre C-hard: İngiltere + Çalışma/İş Vizesi → exact name exclusion
    AND NOT (
      p_country   = 'İngiltere'
      AND p_visa_type = 'Çalışma/İş Vizesi'
  ...
  WHERE  (country IS NULL OR country = p_country)
    ...
    -- Exact name exclusion (ülkeye özgü kaynaklardan da gelmiş olabilir)
    AND NOT (
      p_country   = 'İngiltere'
      AND p_visa_type = 'Çalışma/İş Vizesi'
      AND doc_name = ANY(uk_calisma_excl)
