-- Kural 46: Almanya + Turistik + işveren: şirket dilekçesi sil, şahsi dilekçe + Bağkur ekle
...
BEGIN
  ...
  -- 46. Almanya + Turistik + isveren: sirket dilekce sil, sahsi dilekce + Bagkur ekle
  IF p_country = 'Almanya' AND p_visa_type = 'Turistik' AND p_occupation = 'isveren' THEN
    DELETE FROM public.user_submitted_docs
    WHERE  application_id = p_application_id
      AND  doc_name ILIKE '%şirket antetli kağıdına yazılmış vize talep%';
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (p_application_id, 'Şahsi vize talep dilekçesi (konsolosluğa hitaben yazılmış, seyahat amacını ve tarihlerini belirten, ıslak imzalı mektup)', 'digital', 'pending');
    END IF;
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO public.user_submitted_docs (application_id, doc_name, delivery_type, status)
      VALUES (p_application_id, 'SGK 4B (Bağkur) hizmet dökümü (e-devletten barkodlu, tüm sigorta geçmişini gösteren)', 'digital', 'pending');
    END IF;
  END IF;
