...
export default function AktivitePage() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ background: 'white', borderBottom: '1px solid #e8e4da', padding: isMobile ? '0.75rem 1rem' : '0.875rem 1.5rem', flexShrink: 0 }}>
      ...
      <div style={{ padding: isMobile ? '0.75rem' : '1.5rem', overflowY: 'auto', flex: 1, background: '#faf8f3' }}>
        <div style={{ background: 'white', border: '1px solid #e8e4da', borderRadius: '12px', overflow: 'hidden' }}>
          ...
          {isMobile ? (
            <div>
              {logs.map((log, idx) => (
                <div key={log.id} style={{ padding: '10px 0.875rem', borderBottom: idx < logs.length - 1 ? '1px solid #f0ede6' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ fontSize: '12px', fontWeight: '500', color: '#1a3a5c', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{log.action}</div>
                      <div style={{ fontSize: '11px', color: '#9aaabb', marginTop: '2px' }}>
                        {log.user_name || '-'} · {log.entity_name || '-'}
                      </div>
                    </div>
                    <div style={{ fontSize: '10px', color: '#9aaabb', whiteSpace: 'nowrap', flexShrink: 0 }}>
              ...
              {logs.length === 0 && (
                <div style={{ padding: '2rem', textAlign: 'center', color: '#9aaabb', fontSize: '13px' }}>Henüz aktivite kaydı yok.</div>
              )}
            </div>
          ) : (
