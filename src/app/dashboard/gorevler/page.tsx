  ...
  function GorevItem({ task }: { task: any }) {
    return (
      <div style={{
        ...
        border: '1px solid #f0ede6', marginBottom: '8px',
        background: 'white',
        ...
        <div
          ...
          style={{
            ...
            border: '2px solid #e8e4da', flexShrink: 0, cursor: 'pointer',
        ...
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          {task.client_id && (
            <button
              onClick={() => router.push(`/dashboard/musteriler/${task.client_id}`)}
              style={{ padding: '3px 8px', fontSize: '10px', background: '#1a3a5c', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}
            >
              Profil
            </button>
          )}
          <button
            onClick={() => toggleTask(task)}
            style={{ padding: '3px 8px', fontSize: '10px', background: '#1a7a45', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}
          >
            ✓ Tamamla
          </button>
        </div>
              ...
              <div style={{ background: 'white', border: '1px solid #e8e4da', borderRadius: '12px', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '0.75rem' }}>
                  {acil.length === 0 ? (
                    ...
                    acil.map(t => <GorevItem key={t.id} task={t} />)
                  )}
                </div>
              </div>
            </div>
              ...
              <div style={{ background: 'white', border: '1px solid #e8e4da', borderRadius: '12px', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '0.75rem' }}>
                  {normal.length === 0 ? (
                    ...
                    normal.map(t => <GorevItem key={t.id} task={t} />)
                  )}
                </div>
              </div>
            </div>
